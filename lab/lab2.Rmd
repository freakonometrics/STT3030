...
Dans ce notebook, les champs "..." sont à compléter.
...
Combien y a-t-il d'observations dans ce jeu de données, noté $n$ ? Et, combien de variables explicatives disposons-nous pour prédire $Y$, noté $p$ ?
```{r}
n <- ...
p <- ...
...
A l'exception de "medv", y a-t-il dans le jeu de données "Boston" une variable quantitative ? Une variable qualitative ? Si oui, quel est son nombre de catégories ?
...
cat("Nombre de catégories de la variable chas:", ..., "\n")

unique(Boston$rad)
k2 <- length(unique(Boston$rad))
cat("Nombre de catégories de la variable rad:", ..., "\n")
```
...
# Proportion de données dans la base d'entraînement
prop <- 0.8
prop*nrow(Boston)
n_a <- round(prop*nrow(Boston)) # on utilise round pour avoir un nombre entier
n_a

# Mélange des numéros de lignes de Boston
id <- sample(...)
...
```{r}
cat("Proportion de données d'entraînement:", ..., "%", "\n")
cat("Proportion de données de test:", ..., "%", "\n")
```
...
Affichez les deux coefficients du modèle linéaire ainsi qu'un intervalle de confiance au niveau de confiance 95% sur chacun des coefficients à l'aide de la fonction "confint":
...
On peut également calculer des intervalles de prédiction au niveau de confiance 95% pour les prédictions sur les données de l'échantillon de test.
...
# On fait de même pour les échantillons d'entraînement et de test créés
Boston_a$chas <- ...
Boston_t$chas <- ...
```
...
# L'expression "as.numeric(Boston_t$chas == "1")" nous donne la fonction indicatrice:
# 1{chas = 1}
preds_2[1:5]
```
...
On peut désormais afficher le modèle linéaire appris (i.e. les deux droites obtenues) ainsi que les données de test ("medv" en fonction de "lstat"), en différenciant par couleur sur "chas". Cette fois-ci, on utilise la librairie "ggplot2" pour construire les graphes.
...
```{r}
# utilisez le signe "*" au lieu de "+" pour indiquer la prise en compte de l'intéraction à R
fit_3 <- lm(...) 
summary(fit_3)
```
...
On commence par créer un modèle de régression linéaire à l'aide de toutes les variables présentes dans le jeu de données "Boston" (à l'exception de la variable à prédire "medv"). Le terme "~ ." nous évite d'avoir à renseigner toutes les variables explicatives du modèle linéaire et prend automatiquement l'ensemble des variables de "Boston" (sauf "medv").
...
```{r}
plot(..., pch = 20, xlab = "lstat", ylab = "medv")
```
...
Quelles sont les différentes modalités de cette variable ? Et, selon R, quelle est la modalité de référence ?
...
(sinon, vous pouvez les télécharger sur le github du cours: <https://github.com/freakonometrics/STT3030.git>)
