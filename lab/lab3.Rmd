...
```{r}
hist(..., xlab = "Salary", main = "Histogram of Salary", breaks = 20)
```
...
# Proportion du jeu de données qu'on désire 
# avoir dans l'échantillon d'entraînement (80%):
prop <- 0.8
# Nombre d'observations souhaité dans 
# l'échantillon d'entraînement (80% * n):
ntrain <- floor(nrow(x)*prop) 
# Nombre d'observations dans 
# l'échantillon de test (20% * n):
ntest <- n - ntrain 

# On mélange les numéros de ligne du jeu de données et 
# on conserve les ntrain premières:
...
# (associé à un numéro de colonnes de x_train) 
# pour chaque courbe:
vnat <- coef(fit)
